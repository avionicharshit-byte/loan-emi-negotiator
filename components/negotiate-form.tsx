...
export function NegotiateForm({ onSubmit }: Props) {
    ...
    >
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          The defaults below are an illustrative profile. Replace with your own to get a personalized plan.
        </p>
      </header>
      ...
      <Section title="Loan basics">
        ...
        <Field label="Principal outstanding (₹)" error={errors.principalOutstanding?.message}>
          <Input type="number" step="1000" {...register("principalOutstanding", { valueAsNumber: true })} />
        </Field>
        <Field label="Current rate (%)" error={errors.currentRate?.message}>
          <Input type="number" step="0.01" {...register("currentRate", { valueAsNumber: true })} />
        </Field>
        <Field label="Tenure remaining (months)" error={errors.tenureRemainingMonths?.message}>
          <Input type="number" {...register("tenureRemainingMonths", { valueAsNumber: true })} />
        </Field>
        <Field label="Current EMI (₹)" error={errors.currentEMI?.message}>
          <Input type="number" step="100" {...register("currentEMI", { valueAsNumber: true })} />
        </Field>
        <Field label="Loan sanction date" error={errors.sanctionDate?.message}>
          <Input type="month" {...register("sanctionDate")} />
        </Field>
      </Section>

      <Section title="Your profile">
        <Field label="CIBIL score" error={errors.cibilScore?.message}>
          <Input type="number" {...register("cibilScore", { valueAsNumber: true })} />
        </Field>
        <Field label="On-time EMIs paid" error={errors.emisPaidOnTime?.message}>
          <Input type="number" {...register("emisPaidOnTime", { valueAsNumber: true })} />
        </Field>
        <Field label="Prepayments made (total ₹)" error={errors.prepaymentsTotal?.message}>
        ...
        <Field label="Employer category" error={errors.employerCategory?.message}>
          <select
